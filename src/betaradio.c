...
BetaRadio* beta_radio_construct (GType object_type) {
	...
	self->priv->icon = (_tmp0_ = gtk_status_icon_new_from_stock (GTK_STOCK_MISSING_IMAGE), _g_object_unref0 (self->priv->icon), _tmp0_);
	...
	gtk_menu_shell_append ((GtkMenuShell*) self->priv->menu, (GtkWidget*) ((GtkMenuItem*) (_tmp2_ = g_object_ref_sink ((GtkSeparatorMenuItem*) gtk_separator_menu_item_new ()))));
	_g_object_unref0 (_tmp2_);
	group = beta_radio_getMenu (self, self->priv->menu, group);
	gtk_menu_shell_append ((GtkMenuShell*) self->priv->menu, (GtkWidget*) ((GtkMenuItem*) (_tmp3_ = g_object_ref_sink ((GtkSeparatorMenuItem*) gtk_separator_menu_item_new ()))));
...
static GSList* beta_radio_getMenu (BetaRadio* self, GtkMenu* menu, GSList* group) {
	...
	if (json_soup_isArray (list) == FALSE) {
		result = group;
	...
	length = json_soup_length (list);
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				feed = (_tmp2_ = json_soup_getString (_tmp1_ = json_soup_array (list, i)), _g_object_unref0 (_tmp1_), _tmp2_);
				json = json_soup_new_http (feed);
				if ((_tmp4_ = json_soup_isString (_tmp3_ = json_soup_object (json, "property")) == FALSE, _g_object_unref0 (_tmp3_), _tmp4_)) {
					_g_free0 (feed);
					_g_object_unref0 (json);
					continue;
				}
				title = (_tmp6_ = json_soup_getString (_tmp5_ = json_soup_sibling (json, "title")), _g_object_unref0 (_tmp5_), _tmp6_);
				...
				property = (_tmp8_ = json_soup_getString (_tmp7_ = json_soup_sibling (json, "property")), _g_object_unref0 (_tmp7_), _tmp8_);
				if (_vala_strcmp0 (property, "category") == 0) {
					JsonSoup* _tmp10_;
					_tmp9_ = json_soup_isArray (_tmp10_ = json_soup_sibling (json, "category")) == TRUE;
				...
				if (_tmp9_) {
					...
					if (_vala_strcmp0 (property, "channel") == 0) {
						JsonSoup* _tmp12_;
						_tmp11_ = json_soup_isArray (_tmp12_ = json_soup_sibling (json, "channel")) == TRUE;
	...
	length = json_soup_length (json);
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				category = (_tmp3_ = json_soup_getString (_tmp2_ = json_soup_object (_tmp1_ = json_soup_array (json, i), "title")), _g_object_unref0 (_tmp2_), _g_object_unref0 (_tmp1_), _tmp3_);
				...
				size = (_tmp5_ = json_soup_length (_tmp4_ = json_soup_sibling (json, "channel")), _g_object_unref0 (_tmp4_), _tmp5_);
				{
					...
					j = 0;
					{
						...
						while (TRUE) {
							...
							title = (_tmp9_ = json_soup_getString (_tmp8_ = json_soup_object (_tmp7_ = json_soup_array (json, j), "title")), _g_object_unref0 (_tmp8_), _g_object_unref0 (_tmp7_), _tmp9_);
							type = (_tmp11_ = json_soup_getString (_tmp10_ = json_soup_sibling (json, "type")), _g_object_unref0 (_tmp10_), _tmp11_);
							_data1_->url = (_tmp14_ = beta_radio_filter_url (self, _tmp13_ = json_soup_getString (_tmp12_ = json_soup_sibling (json, "url")), type), _g_free0 (_tmp13_), _g_object_unref0 (_tmp12_), _tmp14_);
							radio = g_object_ref_sink ((GtkRadioMenuItem*) gtk_radio_menu_item_new_with_label (group, title));
	...
	length = json_soup_length (json);
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				title = (_tmp3_ = json_soup_getString (_tmp2_ = json_soup_object (_tmp1_ = json_soup_array (json, i), "title")), _g_object_unref0 (_tmp2_), _g_object_unref0 (_tmp1_), _tmp3_);
				type = (_tmp5_ = json_soup_getString (_tmp4_ = json_soup_sibling (json, "type")), _g_object_unref0 (_tmp4_), _tmp5_);
				_data2_->url = (_tmp8_ = beta_radio_filter_url (self, _tmp7_ = json_soup_getString (_tmp6_ = json_soup_sibling (json, "url")), type), _g_free0 (_tmp7_), _g_object_unref0 (_tmp6_), _tmp8_);
				radio = g_object_ref_sink ((GtkRadioMenuItem*) gtk_radio_menu_item_new_with_label (group, title));
				group = gtk_radio_menu_item_get_group (radio);
